...
export const CartItem = ({ item }) => {
    const { id, title, price, variant, image, quantity } = item;
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex flex-col sm:flex-row gap-6 items-start sm:items-center hover:shadow-md transition-all duration-300 group relative">
            ...
            <div className="flex-grow flex flex-col gap-2 w-full">
                <div className="flex justify-between items-start gap-4">
                    <div>
                        <h3 className="text-lg font-bold text-gray-900 group-hover:text-orange-600 transition-colors line-clamp-1">{title}</h3>
                        <p className="text-sm text-gray-500">{variant || 'Default Variant'}</p>
                    </div>
                    <span className="text-lg font-bold text-gray-900">${numericPrice.toFixed(2)}</span>
                </div>
